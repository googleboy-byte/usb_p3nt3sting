...
# create a log file header
    ...
    #log_to_file(b"Time,User,Executable,CommandLine,PID,ParentPID,Privileges")
...
while True:
    try:
        with open("C:\Downloads\process_monitor_log.txt", "a") as fd:
            ...
            process_log_message = "%s,%s" % (cmdline, privileges)
            #print("%s\r\n" % process_log_message)
            
            fd.write("\n%s" % process_log_message)
